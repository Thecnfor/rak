...
class TricycleChassis(ChassisBase):
    ...
    def params_init(self):
        ...
        # 根据小车三轮的运动计算小车运动， 正解
        self.transform_forward = np.array(
            [
                [0, 2 / 3, 1 / (3 * r)],
                [-1 / (2 * cos_a), -1 / 3, 1 / (3 * r)],
                [1 / (2 * cos_a), -1 / 3, 1 / (3 * r)],
        ...
        # 根据小车运动计算小车三轮的运动， 逆解
        self.transform_inverse = np.array(
            [[0, -cos_a, cos_a], [1, -sin_a, -sin_a], [r, r, r]]
        )
...
class Diff2Chassis(ChassisBase):
    ...
    ********
    ...
    def params_init(self):
        r = self.track
        # 根据小车差速二轮运动计算小车运动， 正解
        self.transform_forward = np.array([[-1 / 2, 0, 1 / r], [1 / 2, 0, 1 / r]])

        # 根据小车运动计算小车差速二轮运动， 逆解
        self.transform_inverse = np.array([[-1, 1], [0, 0], [r / 2, r / 2]])
...
class Diff4Chassis(ChassisBase):
    ...
    ********
    ...
    def params_init(self):
        ...
        # 根据小车运动计算小车差速四轮运动， 逆解
        self.transform_inverse = np.array([[1, -1, -1, 1], [0, 0, 0, 0], [r, r, r, r]])
...
class QuadricycleChassis(ChassisBase):
    ...
    ********
    ...
    def params_init(self):
        ...
        # 根据小车运动计算小车四轮运动, 逆解
        self.transform_inverse = np.array(
            [[s_th, -s_th, -s_th, s_th], [s_th, s_th, -s_th, -s_th], [r, r, r, r]]
        )
...
class CarBase:
    ...
    def set_pose(
        ...
        pose,
        during=None,
        vel=[0.15, 0.15, math.pi / 3],
        threshold=[0.004, 0.004, 0.02],
    ):
        ...
        while True:
            ...
            # print(vel_x_pid)
            # 世界坐标速度转换车子坐标速度
            vel_out = self.sp_world2car(
                [vel_x_pid, vel_y_pid, vel_yaw_out], pose_now[2]
            )
    ...
    def set_pose_offset(
        ...
        pose,
        during=None,
        vel=[0.2, 0.2, math.pi / 3],
        threshold=[0.002, 0.002, 0.02],
    ):
        start_pos = self.chassis.odom.pose
        tar_pos = [0, 0, 0]
        tar_pos[0] = (
            start_pos[0]
            + pose[0] * math.cos(start_pos[2])
            - pose[1] * math.sin(start_pos[2])
        )
        tar_pos[1] = (
            start_pos[1]
            + pose[1] * math.cos(start_pos[2])
            + pose[0] * math.sin(start_pos[2])
        )
