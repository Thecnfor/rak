...
class Button_1:
    ...
    def __init__(self, port, button):
        ...
        self.cmd_data = bytes.fromhex(
            "77 68 05 00 01 DB {} {} 0A".format(port_str, button_str)
        )
...
class UltrasonicSensor_1:
    ...
    def read(self):
        ...
        ultrasonic_sensor = struct.unpack(
            "<f", struct.pack("4B", *return_data_ultrasonic)
        )[0]
...
class ServoBus_1:
    ...
    def set_angle(self, angle, speed):
        ...
        speed = int(speed)
        cmd_servo_data = (
            bytes.fromhex("77 68 08 00 02 36 {}".format(self.port_str))
            + speed.to_bytes(1, byteorder="big", signed=True)
            + angle.to_bytes(4, byteorder="little", signed=True)
            + bytes.fromhex("0A")
        )
    ...
    def set_speed(self, speed):
        ...
        angle = 1
        cmd_servo_data = (
            bytes.fromhex("77 68 06 00 02 37 {}".format(self.port_str))
            + speed.to_bytes(1, byteorder="big", signed=True)
...
class ServoPwm_1:
    ...
    def set_angle(self, angle, speed):
        ...
        if speed < 0:
            ...
            speed = 180
        cmd_servo_data = (
            bytes.fromhex("77 68 06 00 02 0B")
...
class DigitOut_1:
    ...
    def out(self, value):  # 1断 2通
        ...
        cmd_servo_data = bytes.fromhex(
            "77 68 05 00 02 1E {} {} 0A".format(self.port_str, value_str)
        )
        serial_mc601.write(cmd_servo_data)
...
class EncoderMotorAllSim_1:
    ...
    def set_speed(self, speed4: List[int]):
        ...
        self.encoder4 = self.encoder4 + (
            self.speed4 * self.speed_rate * (time.time() - self.last_time)
        ).astype(np.int32)
        # 更新速度
...
class Motor_1:
    ...
    def rotate(self, speed):
        ...
        self.speed = speed
        cmd_servo_data = (
            bytes.fromhex("77 68 06 00 02 0C")
            + bytes.fromhex(self.driver_id_str)
            + bytes.fromhex(self.port_str)
            + speed.to_bytes(1, byteorder="big", signed=True)
            + bytes.fromhex("0A")
        )
    ...
    def get_encoder(self):
        ...
        encoder = self.encoder + self.speed * self.speed_rate * (
...
class EncoderMotor4Sim_1:
    ...
    def set_speed(self, speed4: List[int]):
        ...
        self.speed4 = np.array(speed4)
        self.encoder4 = self.encoder4 + (
            self.speed4_last * self.speed_rate * (time.time() - self.last_time)
        ).astype(np.int32)
        self.last_time = time.time()
...
class Motor4_1:
    ...
    def set_speed(self, speeds: List[int]):
        ...
        for i in range(4):
            sp_bytes = (i + 1).to_bytes(1, byteorder="big") + speeds[i].to_bytes(
...
class Infrared_1:
    ...
    def read(self):
        ...
        infrared_sensor = struct.unpack("<i", struct.pack("4B", *return_data_infrared))[
            0
        ]
...
class AnalogInput_1:
    ...
    def read(self):
        ...
        if (
            len(return_data) != 9
            or return_data[-4] != 0xE1
            or return_data[-3] != self.port
        ):
...
class PortOut_1:
    ...
    def out(self, value):  # 1断 2通
        value_str = "{:02x}".format(value)
        cmd_servo_data = bytes.fromhex(
            "77 68 05 00 02 3A {} {} 0A".format(self.port_str, value_str)
        )
        serial_mc601.write(cmd_servo_data)
...
class NixieTube_1:
    ...
    def set_number(self, value):
        ...
        cmd_servo_data = bytes.fromhex(
            "77 68 06 00 02 38 {} {} {} 0A".format(
                self.port_str, value1_str, value2_str
            )
        )
        serial_mc601.write(cmd_servo_data)
