...
DownloadAddress = 0x8000000 + (
    48 * 1024
)  # 从第48K的地方开始升级固件,此参数后续作为命令行参数
...
def crc32_stm(bytes_arr):
    ...
    while length >= 4:
        ...
        crc = ((crc << 8) & 0xFFFFFFFF) ^ custom_crc32_table[
            0xFF & ((crc >> 24) ^ (v >> 8))
        ]
        crc = ((crc << 8) & 0xFFFFFFFF) ^ custom_crc32_table[
            0xFF & ((crc >> 24) ^ (v >> 16))
    ...
    try:
        ...
        if len(revtemp) == CMD_PING_REV_SIZE:
            ...
            if revtemp[0] == CMD_M32_2_PC_HEAD0 and revtemp[1] == CMD_M32_2_PC_HEAD1:
                if revtemp[3] == CMD_PING and revtemp[
                    CMD_PING_REV_SIZE - 1
                ] == checksum(revtemp):
...
def Ping2ServoControl(device):
    ...
    try:
        ...
        if len(revtemp) == CMD_PING_REV_SIZE:
            # 接收到下位机反馈
            if revtemp[0] == CMD_M32_2_PC_HEAD0 and revtemp[1] == CMD_M32_2_PC_HEAD1:
                if revtemp[3] == CMD_SERVO and revtemp[
                    CMD_PING_REV_SIZE - 1
                ] == checksum(revtemp):
                    control_type = revtemp[6] + revtemp[7] * 256
                    ser.close()
...
def SendCode2M32Buffer(ser, codearray_4k):
    ...
    try:
        ...
        if len(revtemp) == CMD_DW_REV_SIZE:
            # 接收到下位机反馈
            if revtemp[0] == CMD_M32_2_PC_HEAD0 and revtemp[1] == CMD_M32_2_PC_HEAD1:
                if revtemp[3] == CMD_WRITEBUFFER and revtemp[
                    CMD_DW_REV_SIZE - 1
                ] == checksum(revtemp):
                    return True
        return False
        ...
        if len(revtemp) == CMD_RAM2FLASH_REV_SIZE:
            ...
            if revtemp[0] == CMD_M32_2_PC_HEAD0 and revtemp[1] == CMD_M32_2_PC_HEAD1:
                if revtemp[CMD_RAM2FLASH_REV_SIZE - 1] == checksum(revtemp):
                    if (
                        senddata[6] == revtemp[6]
                        and senddata[7] == revtemp[7]
                        and senddata[8] == revtemp[8]
                        and senddata[9] == revtemp[9]
                    ):
            ...
            if revtemp[0] == CMD_M32_2_PC_HEAD0 and revtemp[1] == CMD_M32_2_PC_HEAD1:
                if revtemp[CMD_RAM2FLASH_REV_SIZE - 1] == checksum(revtemp):
                    revcrc = (
                        int(revtemp[6])
                        + int(revtemp[7]) * 0x100
                        + int(revtemp[8]) * 0x10000
                        + int(revtemp[9]) * 0x1000000
                    )
                    ...
                    if revcrc == codearray_4k_crc:  # crc校验相等，则返回true
                        ...
                        PrintLog(
                            5,
                            "---Crc Same,Resume: Flash "
                            + str(hex(revcrc))
                            + " = Code "
                            + str(hex(codearray_4k_crc)),
                        )
                        return True
        return False
    except Exception as e:
    ...
    try:
        for times in range(10):
            ...
            if len(revtemp) == CMD_RUNCODE_REV_SIZE:
                # 接收到下位机反馈
                if (
                    revtemp[0] == CMD_M32_2_PC_HEAD0
                    and revtemp[1] == CMD_M32_2_PC_HEAD1
                ):
...
def Download(PORTNAME, ADDRESS):
    ...
    while True:
        PrintLog(
            4,
            "---Download : "
            + hex(ADDRESS + addressoffset)
            + " "
            + str(int(addressoffset / 1024))
            + "k/"
            + str(int(len(code) / 1024))
            + "k",
        )
...
def ConnectControl():
    ...
    if len(port_list) > 0:
        for i in range(0, len(port_list)):  # 遍历所有串口
            ...
            if (
                port_list[i].pid == PID and port_list[i].vid == VID
            ):  # ch34使用的是USB转串口，故带有VID和PID数据
                # 控制器使用的串口
...
# result（bool)：，True/False
# ScratchMSG(string)：返回下载log
def Scratch_Download_MC602P(
    RunName="RunA", isrun=False, qtcallback=QT_DowloadCallbackDefault
):
    global ScratchMSG
    global qtcallbackCall
    # 下载GUI用户界面程序********************************************
