...
class CotrollerInfo:
    ...
    def __str__(self) -> str:
        return "baudrate:{},timeout:{},mode:{}".format(
            self.baudrate, self.timeout, self.connect_mode
        )
...
class MC602Wireness(CotrollerInfo):
    ...
    def send_cmd(self, serial_obj, cmd: bytes):
        ...
        # 转义处理
        cmd_data_escape = cmd_data.replace(self.header, self.header_escape).replace(
            self.tail, self.tail_escape
        )
        # 加入头尾数据帧
    ...
    def get_anwser(self, serial_obj, time_out=0.15):
        ...
        while True:
            if time.time() - time_start > time_out:
                return None
            # logger.info("get_anwser:\'{}\'".format(res.hex(' ')))
            res = res.replace(self.header_escape, self.header).replace(
                self.tail_escape, self.tail
            )
            rx_len = len(res)
            if rx_len == dst_len:
                if res[0] == self.header[0] and res[-1] == self.tail[0]:
