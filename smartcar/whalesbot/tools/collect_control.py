...
class DataCollector:
    ...
    def clear_all(self):
        ...
        try:
            subprocess.run(
                ["find", self.save_dir, "-name", "*.jpg", "-delete"], check=True
            )
        except:
            pass
        ...
        while not self.exit_flag:
            if self.run_flag:
                try:
                    ...
                    # 2. 生成文件名
                    img_name = (
                        (name_length - len(str(self.index))) * "0"
                        + str(self.index)
                        + ".jpg"
                    )
                    ...
                    # 4. 保存数据
                    data_dict = {"img_path": img_name, "state": self.car_state.copy()}
...
class CollectControlCar:
    def __init__(
        ...
        self.collector1 = DataCollector(camera=self.cap1, save_dir=dir1, cam_id="cam1")
    ...
    def close(self):
        ...
        self.exit_flag = True
        self.display.show(
            f"cam1_num: {self.collector1.index}\ncam2_num: {self.collector2.index}\n"
        )
