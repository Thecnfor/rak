...
def logger_handler(name: str) -> logging.Logger:
    ...
    __all_log_path = os.path.join(
        log_dir, now_time + "-all" + ".log"
    )  # 收集所有日志信息文件
    __error_log_path = os.path.join(
        log_dir, now_time + "-error" + ".log"
    )  # 收集错误日志信息文件
    ...
    handler_cfgs = [
        {
            "type": "file",
            "filename": __all_log_path,
            "level": logging.INFO,
            "formatter": formatter,
        },
        {
            "type": "file",
            "filename": __error_log_path,
            "level": logging.ERROR,
            "formatter": formatter,
        },
