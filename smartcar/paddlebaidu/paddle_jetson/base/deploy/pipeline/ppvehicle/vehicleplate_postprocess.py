...
class DBPostProcess(object):
    ...
    def box_score_fast(self, bitmap, _box):
        ...
        h, w = bitmap.shape[:2]
        box = _box.copy()
        xmin = np.clip(np.floor(box[:, 0].min()).astype(np.intp), 0, w - 1)
        xmax = np.clip(np.ceil(box[:, 0].max()).astype(np.intp), 0, w - 1)
        ymin = np.clip(np.floor(box[:, 1].min()).astype(np.intp), 0, h - 1)
        ymax = np.clip(np.ceil(box[:, 1].max()).astype(np.intp), 0, h - 1)
