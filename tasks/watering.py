...
DETECT_POSE = dict(
    x=-0.200, y=DETECT_Y, arm=ARM_TOWER, hand=-70.0
)  # 识别水塔姿势 (编排层预摆, 任务只校验)
...
DELIVER_Y = [-0.035, -0.080, -0.120]  # 放块第1/2/3层 y (梯度)
DELIVER_HAND = [-90, -90, -90]  # 放块第1/2/3层手爪
CARRY_X = [[-0.075, -0.070, -0.070], [-0.075, -0.070, -0.070]]  # 每塔每块放块 X (m)
...
PICK = dict(  # 机械臂伺服抓水块(期望点)
    cx=-0.145,
    cy=-0.425,  # 大臂-1/滑轨-1 (跟 seeding 抓取一致)
)
# ----- 分段伺服参数(粗对齐→精对齐, 照抄 seeding._align_staged 规则) -----
# 粗对齐: 大增益(0.35/0.25)快速把水块拉近 + 大死区(0.10), 4s 超时, settle 4 帧即粗到位,
#         lock 5 帧首次锁定。
# 精对齐: 小增益(0.20/0.20)小死区(0.02)精确收敛, 6s 超时, settle 4 帧,
...
def _chassis(car, pos, target):
    ...
    if abs(dx) < 0.005:
        pos[0] = target
        return
    car.move_for([dx, 0, 0], max_velocities=CHASSIS_V, tolerance=[0.002, 0.002, 0.01])
    pos[0] = target
...
def _check_detect_pose(car):
    ...
    if ax and ay and aa:
        print(
            f"[机械臂] 已处于识别水塔姿势 (x={DETECT_POSE['x']}, y={DETECT_POSE['y']}, "
            f"大臂={DETECT_POSE['arm']}, 末端={car.arm.hand_angle})"
        )
        return
    print(
        f"[机械臂] 不在识别水塔姿势 (x={car.arm.x_pose_now:.3f}, y={car.arm.y_pose_now:.3f}, "
        f"大臂={car.arm.angle}, 末端={car.arm.hand_angle}), 按安全顺序兜底摆回"
    )
    _arm_to(
        car, DETECT_POSE["x"], DETECT_POSE["y"], DETECT_POSE["arm"], DETECT_POSE["hand"]
    )
...
def _align_tower(car, label=None):
    """底盘视觉对齐水塔: label 已由保底探测给出则直接用; 否则等 1.8s 找任一水标签.
    只前后(横向锁死),"""
    if label is None:
        end = time.time() + 1.8
        while time.time() < end:
...
def run_one_tower(car, tower_idx, is_last_tower=False):
    ...
    direction = (
        1.0 if tower_idx == 0 else -1.0
    )  # 塔1水块在前方(向前拿), 塔2在后方(向后拿)
    for k in range(n):
        print(f"  搬第 {k+1}/{n} 块")
        # ===== 抓 =====
        group = k // 2  # 每 2 块一组
        ...
        _arm_to(
            car, pick_x, PICK_POSE_Y, ARM_SHELF, PICK_HAND
        )  # 转大臂+93/末端, X→指定距离
...
def run(car):
    ...
    try:
        ...
        car.arm.grasp(False)
        car.arm.set_arm_angle(
            ARM_SHELF
        )  # 大臂回 LEFT(朝置物架侧), 与编排层 TASK_END_POSE 一致
        car.stop()
