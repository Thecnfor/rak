...
PICK_POSE_Y  = -0.180                        # 抓块姿态 y (servo_y)  [test 校准]
PICK_HAND    = -20.0                            # 抓块姿态手爪
FIRST_CUBE_X, SECOND_CUBE_X = -0.145, -0.220 # 每块组内 第1/第2 块 X
GRASP_Y, LIFT_Y = -0.060, -0.150             # 吸块下降 y / 吸完抬回 y  [GRASP_Y test 校准]
...
TRACK = dict(                                   # 底盘对齐水塔 (只前后; sign 不传, 按大臂档位自动定)  [test 校准 cx/cy/v_max]
    cx=0.060, cy=-0.460, kp=(0.0, 0.28),
    deadband=0.04, hold=6,
    v_max=0.11, timeout=15.0,
)
PICK = dict(                                    # 机械臂伺服抓水块  [test 校准 cx/cy/gains]
