...
- `smartcar/paddlebaidu/` is the perception and language layer. `infer_cs` provides the client interface used by `MyCar`; only two inference backends are configured — `lane` (LaneInfer, port 5001) and `task` (YoloeInfer, port 5002). The former `front`/`ocr` services were removed and OCR is disabled in code (`ocr_rec` stays `None`, so `get_ocr`/`get_det_ocr` are inert). `ernie_bot` wraps image/order analysis and prompt handling. Model assets live under `smartcar/paddlebaidu/models/` (`lane_model`, `task2026`).
