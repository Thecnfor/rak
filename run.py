...
TASK_END_POSE = {
    ...
    "target_detection": (
        ...
        "RIGHT",
        "UP",
    ),  # 对齐 watering.DETECT_POSE; 旧值 -0.3 不可达致 arm Timeout
...
class _CompetitionOrchestrator(Orchestrator):
    ...
    def _restart_process(self) -> None:
        """按 3 全局急停: 刹停后非零退出, systemd Restart=on-failure 自动重拉进程。

        任务 run() 里有不查 _stop_flag 的开环动作(move_time/set_velocity_for_duration),
        光设标志停不住; 直接退出让 systemd 重拉才能"立即停一切 + 全新 init 回初始状态"。
        选 systemd 重拉而非 os.execv: exec 会继承残留按键/文件描述符(实测卡死在
        key_thread.join), systemd 重拉 5s 后进程彻底重建, 干净且是验证过的路径。
        """
        ...
        try:
            ...
            print(f"刹停异常: {e}")
        # 非零退出码 → systemd Restart=on-failure 重拉 (StartLimitBurst 已放宽)
        os._exit(42)

    def _key_loop(self):
        """全局按键采集: 3 全局急停(任何状态立即退出重启), 其余键入队给编排器消费."""
        while self.running:
            try:
                event = self._key.read()
                if event and event <= 4:
                    if event == self.KEY_EMERGENCY:
                        # 3: 立即刹停并退出(无论等待/巡航/任务中); 本函数不再返回
...
def main():
    ...
    car = create_car(
        reset=True, comp_mode=True, stream=no_stream
