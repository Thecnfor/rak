...
build_one() {
    local name=$1 model_dir=$2 model_file=$3 params_file=$4 shapes=$5
    ...
    timeout 900 $TRTEXEC --onnx="$onnx" --saveEngine="$engine" --fp16 \
...
# lane: CLRNet, 输入 1x3x128x128
build_one lane lane_model cnn_lane.pdmodel cnn_lane.pdiparams "inputs:1x3x128x128"
# task: PP-YOLOE 检测, 输入 batch=1 (模型导出声明仅支持 batch==1)
build_one task task2026 model.pdmodel model.pdiparams "inputs:1x3x640x640"
